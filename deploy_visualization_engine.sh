...
echo "================================================================="
echo "📊 Deploying Production Visualization Engine..."
echo "================================================================="

# 1. Ensure required matplotlib and parsing stack is present
uv pip install matplotlib pandas numpy

# 2. Write the complete, production-ready extraction and plotting engine
cat <<'EOF' >python/crystalmath/server/handlers/visualization.py
import os
import re
import pathlib
                ...
                for line in f:
                    ...
                    if "TOTAL ENERGY(AU)" in line or "E(AU)" in line:
                        ...
                        try:
                            ...
                            steps.append({"step": cycle, "energy": val * 27.211386}) # Convert Hartree to eV
...
def handle_generate_plot_image(job_id: int, plot_type: str, cache_dir: str, db: Any) -> Dict[str, Any]:
    """
    ...
    if not steps_data:
        return {"status": "error", "message": "No numerical vector metrics resolved for this calculation log."}
    ...
    if plot_type == "convergence":
        ...
        ax.plot(x, y, color='#89b4fa', marker='o', markersize=4, linestyle='-', linewidth=1.5, label='Total Energy')
        ax.set_xlabel("Optimization Step", color='#cdd6f4', fontsize=10)
        ax.set_ylabel("Energy (eV)", color='#cdd6f4', fontsize=10)
        ax.set_title(f"Job #{job_id} ({data_res['data']['code'].upper()}) Trajectory Tracking", color='#cdd6f4', fontsize=12)
...
# 3. Inject routing facades straight into the primary api.py controller module
python3 -c "
path = 'python/crystalmath/api.py'
with open(path, 'r') as f:
    content = f.read()

if 'get_job_visualization_data_json' not in content:
    injection = \"\"\"
...
\"\"\"
    # Append safely to the root class structure context
    with open(path, 'a') as f:
        f.write('\n' + injection + '\n')
    print('✅ Facade bridge endpoints safely attached to api.py context.')
"

echo "================================================================="
echo "✨ Deployment complete! Python extraction boundaries are fully live."
echo "================================================================="
