...
echo "=== Rewriting IPC Framing to Async Tokio Integration ==="

# 1. Overwrite src/ipc/framing.rs with proper async read/write definitions
...
pub async fn write_message<W: AsyncWrite + Unpin>(mut stream: W, payload: &[u8], message_type: u16) -> IoResult<()> {
...
# 2. Patch client.rs line 606 to use the updated parameter signature (.as_bytes() and default msg_type)
# We safely update the exact block matching your compiler output.
sed -i '' 's/write_message(&mut self.writer, \&request_json)/write_message(\&mut self.writer, request_json.as_bytes(), 1)/g' src/ipc/client.rs
